...
def try_upload_offline_data():
    ...
    for filename in os.listdir(OFFLINE_DIR):
        if filename.endswith(".jpg") and "_detected" not in filename:
            ...
            try:
                ...
                if os.path.exists(detected_path):
                    ...
                    print(f"☁️ Uploaded detected image for: {timestamp}")

                # Upload sensor + growth data if exists
                if os.path.exists(json_path):
                    ...
                    ref.set(data)
                    os.remove(json_path)
                    print(f"☁️ Uploaded log data for: {timestamp}")

                os.remove(image_path)
                print(f"🧹 Cleaned offline files for: {timestamp}")

            except Exception as e:
                print(f"⚠️ Failed to upload offline data for {timestamp}: {e}")
