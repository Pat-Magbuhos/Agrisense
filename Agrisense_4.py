...
if not firebase_admin._apps:
    ...
    firebase_admin.initialize_app(cred, {
        ...
        'storageBucket': 'agrisense-6a089.appspot.com'  # Firebase Storage bucket URL
    })
else:
    print("Firebase has already been initialized.")

# Initialize Firebase Storage explicitly with the bucket name
bucket = firebase_admin.storage.bucket('agrisense-6a089.appspot.com')
...
def process_image(raw_image_path, timestamp, cm_per_pixel):
    ...
    try:
        ...
        growth_log_path = os.path.join(OFFLINE_DIR, f"{timestamp}_growth.json")
        with open(growth_log_path, "w") as f:
            import json
            json.dump(growth_data, f)

        # Save placeholder environment data
        env_data = {
            "water_temp": None,
            "light": None,
            "air_temp": None,
        ...
        print(f"📄 Saved growth and environment logs offline: {growth_log_path}, {env_log_path}")
        log(f"✅ Saved offline .json files for {timestamp}")
