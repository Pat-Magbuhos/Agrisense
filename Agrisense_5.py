...
# Function to estimate leaf area using the new model
def estimate_largest_leaf_area(cropped_img, cm_per_pixel):
    """
    Detect leaves in the cropped plant image and return
    the area (cm²) of the largest detected leaf.
    """
    try:
        # Run leaf detection model on the cropped image
        leaf_results = leaf_model.predict(cropped_img, conf=0.3, verbose=False)

        if len(leaf_results[0].boxes) == 0:
            print("No leaves detected.")
            return 0.0  # No leaves found

        # Calculate area for each detected leaf
        leaf_areas = []
        for bbox in leaf_results[0].boxes.xyxy.cpu().numpy():
            x1, y1, x2, y2 = bbox[:4]
            pixel_width = x2 - x1
            pixel_height = y2 - y1
        ...
        print(f"Error estimating largest leaf area: {e}")
        return 0.0
...
def process_image(raw_image_path, timestamp, cm_per_pixel):
    ...
    try:
        ...
        for i, plant_bbox in enumerate(plant_boxes):
            ...
            # Estimate the height of the plant using trigonometry
            est_height = estimate_height(plant_bbox)

            # Calculate the largest leaf area from the detected leaves
            largest_leaf_area = estimate_largest_leaf_area(leaves_in_plant, cm_per_pixel)

            # Classify the growth stage based on the metrics
            growth_stage = classify_growth(est_height, leaf_count, largest_leaf_area)

            # Annotate output image with growth classification and leaf count
            label = f"{growth_stage} | Leaves: {leaf_count}"
            ...
            leaf_data_per_box.append({
                ...
                "largest_leaf_area": largest_leaf_area,  # Add largest leaf area per plant
                "growth_stage": growth_stage  # Save the growth stage per plant
            })
        ...
        if unassigned_leaves:
            ...
            leaf_area = estimate_largest_leaf_area(unassigned_leaves, cm_per_pixel)

            growth_stage = classify_growth(est_height, leaf_count, leaf_area)
            ...
            leaf_data_per_box.append({
                ...
                "leaf_count": leaf_count,
                "largest_leaf_area": leaf_area,  # Include the largest leaf area for ungrouped leaves
                "growth_stage": growth_stage  # Save the growth stage for ungrouped leaves
            })

        # Upload growth parameters to Firebase (Include sensor data)
        growth_data = {
            "height_cm": est_height,
            "leaf_count": sum(leaf['leaf_count'] for leaf in leaf_data_per_box),
            "leaf_area_cm2": sum(leaf['largest_leaf_area'] for leaf in leaf_data_per_box),
