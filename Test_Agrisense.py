...
try:
    ...
    cm_per_pixel = 0.038666  # fallback
    focal_length = 800        # fallback
...
def estimate_largest_leaf_area(leaf_boxes, cm_per_pixel):
    if not leaf_boxes:
        return 0.0
    areas = []
...
def classify_growth(height_cm, leaf_count, leaf_area_cm2):
    ...
    # Decide by maximum matching stage
    if scores["mature"] >= 2:
        return "Mature"
    ...
    for filename in os.listdir(OFFLINE_DIR):
        if filename.endswith(".jpg") and "_detected" not in filename and "_error" not in filename:
            ...
            try:
                ...
                # Upload growth_parameters to Firebase Storage
                ...
                # Upload environment_data to Firebase Storage
...
def process_image(raw_image_path, timestamp, cm_per_pixel):
    ...
    try:
        ...
        # Run plant and leaf detection models
        ...
        # Analyze per Plant
        for plant_bbox in plant_boxes:
            plant_counter += 1
            x1p, y1p, x2p, y2p = plant_bbox

            leaves_in_plant = []
            for idx, leaf_bbox in enumerate(leaf_boxes):
                if idx in assigned_leaves:
                    continue
                if is_inside(plant_bbox, leaf_bbox):
                    leaves_in_plant.append(leaf_bbox)
                    assigned_leaves.add(idx)

            leaf_count = len(leaves_in_plant)
            largest_leaf_area = estimate_largest_leaf_area(leaves_in_plant, cm_per_pixel)
            est_height = estimate_height(plant_bbox)
            growth_stage = classify_growth(est_height, leaf_count, largest_leaf_area)

            growth_stage_label = f"{growth_stage} | Leaves: {leaf_count}"
            (tw, th), baseline = cv2.getTextSize(growth_stage_label, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)

            # Calculate center X position for the label
            center_x = x1p + (x2p - x1p) // 2
            text_x = center_x - tw // 2
            text_y = y1p - 10  # 10 pixels above the bounding box

            # Ensure text stays inside image
            text_x = max(text_x, 0)
            text_y = max(text_y, 20)

            # Draw filled background rectangle behind text
            cv2.rectangle(output_image, (text_x - 2, text_y - th - 2), (text_x + tw + 2, text_y + baseline + 2), (0, 255, 0), -1)
            # Draw text itself
            cv2.putText(output_image, growth_stage_label, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
            # Draw bounding box
            cv2.rectangle(output_image, (x1p, y1p), (x2p, y2p), (0, 255, 0), 2)

            # ✅ FIX: Append this plant's data to the JSON structure
            leaf_data_per_box.append({
                "plant_number": plant_counter,
        ...
        # Upload to Firebase or store locally
        growth_data = {
        ...
        print(f"Uploading growth data to Firebase...")
        upload_json(growth_data, "growth_parameters", timestamp)  # Upload JSON data to Firebase
        print(f"✅ Uploaded growth parameters to Firebase.")

        # Upload images to Firebase
        print(f"Uploading raw image to Firebase...")
        ...
        print(f"Uploading detected image to Firebase...")
        ...
        save_offline(raw_image_path, timestamp)  # Save offline if error occurs
        return None, None, pest_name, growth_stage
